...
usage() {
  cat <<'EOF'
Usage:
  pic_power_capture.sh -- <command> [args...]

Environment:
  MNN_POWER_API_URL        eperf power API. Defaults to http://192.168.101.14:8766.
  MNN_POWER_DEVICE         jetson, rhino, orangepi, orangepi5plus, oneplus13t, or explicit serial.
  ...
  MNN_POWER_MAX_DURATION_SEC Capture safety limit. Defaults to 1800.
...
MAX_DURATION_SEC="${MNN_POWER_MAX_DURATION_SEC:-${MAX_DURATION_SEC:-1800}}"
...
case "${POWER_DEVICE_NORM}" in
  jetson|nvidia_jetson)
    ...
    ;;
  rhino|rhinopi|rhino_pi|rhino_pi_x1|aidlux|adreno)
    START_DEVICE=""
    DEFAULT_MONITOR_TYPE="df"
    DEFAULT_SERIAL="1A5D43"
    ;;
  orangepi|orangepi_mali|orangepi5plus|orange_pi_5_plus|orange_pi5_plus|opi5plus|opi_5_plus)
...
stop_capture() {
  ...
  if ! curl --noproxy '*' -fsS "${csv_url}" -o "${OUTPUT_CSV}"; then
    echo "[power-api] CSV download failed: ${csv_url}" >&2
    return 1
  fi

  local png_path render_error_path
  png_path="${OUTPUT_CSV%.*}.png"
  render_error_path="${OUTPUT_CSV%.*}.render_error.txt"
  if "${SCRIPT_DIR}/render_power_csv.sh" "${OUTPUT_CSV}" >/tmp/mnn_pic_power_render_$$.log 2>&1; then
    rm -f "${render_error_path}" /tmp/mnn_pic_power_render_$$.log
    echo "[power-api] PNG saved: ${png_path}"
  else
    cat /tmp/mnn_pic_power_render_$$.log > "${render_error_path}" 2>/dev/null || true
    rm -f /tmp/mnn_pic_power_render_$$.log
    echo "[power-api] PNG render failed; see ${render_error_path}" >&2
  fi
