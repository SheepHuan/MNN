...
```bash
cd transformers/pic_llm/export

# 测试纯文本（应该仍然正确）
conda run -n kvshare-edge python llmexport.py --path /path/to/model --test "你好"

# 测试图片输入
conda run -n kvshare-edge python llmexport.py --path /path/to/model --test "<img>/path/to/test.jpg</img>描述一下这张图片"
```
...
3. 在 `transformers/pic_llm/engine/src/omni.cpp` 的 `audioProcess()` 中添加 dispatch 分支
...
# 测试音频输入（如果有测试音频文件）
conda run -n kvshare-edge python llmexport.py --path /path/to/model --test "请描述这段音频 <audio>/path/to/test.wav</audio>"
```
...
LD_LIBRARY_PATH="$MNN_ARTIFACT_ROOT/lib:${LD_LIBRARY_PATH:-}" \
"$MNN_ARTIFACT_ROOT/bin/pic_llm_demo" /path/to/MODEL/config.json /tmp/prompt.txt
...
conda run -n kvshare-edge python -c "... model.generate(input_ids, pixel_values, ...) ..."
# 期望输出示例："一位女性和一只金毛犬在沙滩上"

# 2. MNN Python --test
conda run -n kvshare-edge python llmexport.py --path /path/to/model --test "<img>test.jpg</img>描述图片"
# 期望：输出质量与 HF 基准相当

# 3. C++ pic_llm_demo
echo "<img>test.jpg</img>描述图片" > test.txt
LD_LIBRARY_PATH="$MNN_ARTIFACT_ROOT/lib:${LD_LIBRARY_PATH:-}" \
"$MNN_ARTIFACT_ROOT/bin/pic_llm_demo" config.json test.txt
# 期望：输出质量与 HF 基准相当
```
