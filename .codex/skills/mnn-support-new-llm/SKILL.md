---
name: mnn-support-new-llm
description: 当用户要求在本 MNN 仓库的 transformers/pic_llm 中新增、适配或诊断新的 LLM/多模态 LLM 模型支持时使用；覆盖 HuggingFace/ModelScope 模型分析、model_mapper 映射、Hook 对齐测试、PIC LLM 导出和 C++ 验证。
---
...
本 Skill 指导 Codex 在 `transformers/pic_llm` 中为 MNN PIC LLM 添加新的 LLM 模型支持。整个流程采用 **TDD（测试驱动）模式**，每一步都有明确的测试标准，**必须通过当前步骤的测试后才能进入下一步**。
...
| 文件路径 | 作用 | 修改频率 |
|---------|------|---------:|
| `transformers/pic_llm/export/utils/model_mapper.py` | 模型字段映射 | **几乎每个新模型** |
| `transformers/pic_llm/export/utils/model.py` | 统一模型类 `LlmModel` | 偶尔 |
| `transformers/pic_llm/export/utils/transformers.py` | Attention/Decoder/Rotary 等组件 | 新架构时 |
| `transformers/pic_llm/export/utils/config.py` | 模型配置类 `LlmConfig` | 偶尔 |
| `transformers/pic_llm/export/utils/vision.py` | Vision Encoder 实现 | 视觉模型 |
| `transformers/pic_llm/export/utils/audio.py` | Audio Encoder 实现 | 音频模型 |
| `transformers/pic_llm/export/utils/custom_op.py` | 自定义算子导出 | 新算子时 |
| `transformers/pic_llm/export/llmexport.py` | 导出主流程入口 | 偶尔 |
...
**现在请打开 `.codex/skills/mnn-support-new-llm/step1-analyze.md`，开始步骤 1。**
