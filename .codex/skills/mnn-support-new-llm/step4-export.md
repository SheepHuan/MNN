...
优先使用本仓库 Codex 导出脚本。它会解析 ModelScope/HF 本地路径、把输出放到 `.cache/`，并自动使用当前平台 artifacts 中的 `MNNConvert`：
...
如果需要手动调 exporter，仍然从仓库根目录执行，并显式传当前平台的 converter：
...
conda run -n kvshare-edge python transformers/pic_llm/export/llmexport.py \
  --path /path/to/model \
  --export mnn \
  --dst_path .cache/pic-llm-export/MODEL \
...
```bash
JOBS=6 CUDA_ARCHS=72 bash project/linux/build_on_jetson.sh
```
...
"$MNN_ARTIFACT_ROOT/bin/pic_llm_demo" .cache/pic-llm-export/MODEL/config.json /tmp/prompt.txt
```
...
2. **检查 FakeLinear axis 问题**：用 `MNNConvert` 导出 JSON 模型图，搜索 `GatherElements`/`TopKV2` 等 op 的 axis 参数，确认在 3D shape 下仍指向正确维度。详见 `common-pitfalls.md` 第 10 节。
...
| 工具 | 用途 | 命令 |
|------|------|------|
| `MNNConvert` | 将 MNN 模型导出为 JSON，检查 op 图结构和 axis 参数 | `$MNN_ARTIFACT_ROOT/bin/MNNConvert -f MNN --modelFile model.mnn --JsonFile model.json` |
