...
static uint32_t hpc_gpu_adreno_read_sysfs_gpu_id(void) {
  FILE *file = fopen("/sys/class/kgsl/kgsl-3d0/gpu_model", "r");
  ...
  const char *adreno = strstr(model, "Adreno");
  if (adreno == NULL) return 0;
  adreno += strlen("Adreno");

  char *end = NULL;
  const unsigned long product = strtoul(adreno, &end, 10);
  if (end == adreno || product < 500 || product >= 800) return 0;
...
uint32_t hpc_gpu_adreno_ioctl_get_gpu_device_id(int gpu_device) {
  ...
  // Newer KGSL kernels populate gpu_id with the public Adreno product number
  // (e.g. 740). Older kernels only exposed the encoded chip_id, which is the
  // format handled below for compatibility with the original HPC snapshot.
  if (devinfo.gpu_id >= 500 && devinfo.gpu_id < 800) return devinfo.gpu_id;

  // Some modern KGSL kernels expose only the encoded chip_id in the ioctl
  // payload. The legacy byte decoder cannot identify A7xx (for example,
  // 0x43050a01 becomes 6760), while gpu_model remains stable and reports
  // names such as "Adreno740v2". Prefer that public product number.
  const uint32_t sysfs_gpu_id = hpc_gpu_adreno_read_sysfs_gpu_id();
  if (sysfs_gpu_id != 0) return sysfs_gpu_id;

  if (devinfo.gpu_id != 0) return devinfo.gpu_id;
