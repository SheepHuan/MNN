...
class Attention(torch.nn.Module):
    def __init__(self, attn, layer_id, config, rotary, mapper):
        ...
        self.use_paged_attention = getattr(config, 'paged_attention', False)
        attention_cls = PagedAttention if self.use_paged_attention else FusedAttention
        attention_name = 'PagedAttention' if self.use_paged_attention else 'FusedAttention'
        self.fused_attn = attention_cls(self.num_heads * self.head_dim, self.kv_cache,
                                        f'/layers.{layer_id}/self_attn/{attention_name}', layer_id, kv_shared_idx)
