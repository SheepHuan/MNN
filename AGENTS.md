...
.codex/skills/mnn-device-io-bench/SKILL.md  Jetson/OrangePi/AidLux 存储与 DRAM 读取带宽基线测试
...
- 测试 Jetson、Orange Pi 5 Plus、AidLux/Adreno 的 NVMe/SSD/UFS/块设备型号、`fio` 磁盘读取速度、`lmbench bw_mem rd` 内存读取带宽或 STREAM 持续内存带宽：读 `.codex/skills/mnn-device-io-bench/SKILL.md`。
- CPU/CUDA 算子性能优化、分析算子慢因，或在本机交叉编译产物并推到 Jetson 验证：读 `.codex/skills/mnn-opt-ops/SKILL.md`。
- PIC/PagedAttention 的 OpenCL/CUDA/Adreno 优化、OrangePi OpenCL cacheblend/epic/kvshare sparse prefill、Jetson CUDA sparse flash / compact dense、Rhino Adreno mixed K/V / dense family、score layer 前后 full compute 与 sparse compute 边界分析、`PagedAttentionBufExecution.cpp` / `attention_buf.cl` / `softmax_buf.cl` / `PagedAttentionExecution.cu` 热点定位：读 `.codex/skills/mnn-pic-optimize/SKILL.md`。
