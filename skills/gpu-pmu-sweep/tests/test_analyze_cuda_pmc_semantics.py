...
MODULE = load(
    "analyze_cuda_pmc_semantics_wrapper",
    ROOT / "scripts" / "analyze_cuda_pmc_semantics.py",
)
...
class PmcInterpreterCliTest(unittest.TestCase):
    def test_cuda_cli_writes_structured_report_and_markdown(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            operator_cases.write_text(json.dumps({"cases": cases}), encoding="utf-8")
            ...
            result = MODULE.main([
                "--pmc-csv", str(rows),
                "--latency-json", str(latency),
                "--operator-cases", str(operator_cases),
                "--output-json", str(output),
                "--output-md", str(markdown),
                "--minimum-group-samples", "3",
            ])
            ...
            self.assertIn("descriptive_canonical", payload["canonical_metric_sets"])
            self.assertEqual(
