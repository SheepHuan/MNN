    ...
    static struct Reg {
        Reg() {
            ...
            add({"topkv2", "topkv2_buf_fp32", {
                TS{"3.6.0", "topkv2_buf", 2,1, 3,0,2,0, 2, "identity_fp32"},
            }});

            // scale_buf_fp32: 1.2.0 (sizeConst×2,buf×3,buf,int4), 3.6.0 (sizeConst×2,buf×3,buf,int×3)
            add({"scale", "scale_buf_fp32", {
                TS{"1.2.0", "scale_buf", 3,1, 0,0,2,1, 2, "identity_fp32"},
                TS{"3.6.0", "scale_buf", 3,1, 3,0,2,0, 2, "identity_fp32"},
            }});

            // interp_buf_fp32: (sizeConst×2, buf, buf, float×4, int×5) — 1.2.0 + 3.6.0
            add({"interp", "interp_buf_fp32", {
                TS{"1.2.0", "nearest_buf", 1,1, 5,4,2,0, 2, "identity_fp32"},
                TS{"3.6.0", "nearest_buf", 1,1, 5,4,2,0, 2, "identity_fp32"},
            }});

            // range_buf: (sizeConst×2, buf×2, buf, int) — 3.6.0
            add({"range", "range_buf_fp32", {
                TS{"3.6.0", "range_buf", 2,1, 1,0,2,0, 2, "identity_fp32"},
            }});

            // select_buf: (sizeConst×2, buf×3, buf) — 3.6.0
            add({"select", "select_buf_fp32", {
                TS{"3.6.0", "select_buf", 3,1, 0,0,2,0, 2, "identity_fp32"},
            }});
            ...
            add({"winograd", "winogradTransform_buf_fp32", {
                TS{"1.2.0", "winoTransSrcBuf2_3_1", 1,1, 8,0,2,0, 2, "identity_fp32"},
                TS{"3.6.0", "winoTransWeightBuf2_3_1", 1,1, 4,0,2,0, 2, "identity_fp32"},
            }});

            // winogradTransform_subgroup_buf: (sizeConst×2, buf, buf, int×8) — 3.6.0
            add({"winograd", "winogradTransform_subgroup_buf_fp32", {
                TS{"3.6.0", "winoTransSrcBuf2_3_1_c16_c16", 1,1, 8,0,2,0, 2, "identity_fp32"},
            }});

            // unary_subgroup_buf: (sizeConst×2, buf, buf, int×8) — 3.6.0
            add({"unary", "unary_subgroup_buf_fp32", {
                TS{"3.6.0", "unary_buf_c4_c4", 1,1, 8,0,2,0, 2, "identity_fp32"},
            }});
            ...
            add({"grid_sample", "grid_sample_buf_fp32", {
                TS{"3.6.0", "nearest_buf", 2,1, 5,0,2,0, 2, "identity_fp32"},
            }});
            ...
            add({"conv", "conv_2d_buf_fp32", {
                TS{"1.2.0", "conv_2d_c4h1w1", 3,1, 2,0,2,5, 2, "identity_fp32"},
                TS{"3.6.0", "conv_2d_1x1_local", 4,1, 6,0,2,0, 2, "identity_fp32"},
            }});

            // conv_2d_c1_subgroup_buf: (sizeConst×2, buf×4, buf, int×13) — 3.6.0
            add({"conv", "conv_2d_c1_subgroup_buf_fp32", {
                TS{"3.6.0", "conv_2d_buf_subgroup_c1_c4_b2", 4,1, 13,0,2,0, 2, "identity_fp32"},
            }});

            // conv_2d_c16_subgroup_buf: (sizeConst×2, buf×4, buf, int×13) — 3.6.0
            add({"conv", "conv_2d_c16_subgroup_buf_fp32", {
                TS{"3.6.0", "conv_2d_buf_subgroup_c16_c4_b2", 4,1, 13,0,2,0, 2, "identity_fp32"},
            }});

            // conv_2d_int_buf: (sizeConst×2, buf×5, buf, int2, int×3, int2×4, int×3, float) — 3.6.0
            add({"conv", "conv_2d_int_buf_fp32", {
                TS{"3.6.0", "conv_2d_int_c4h1w1", 5,1, 3,1,2,5, 2, "identity_fp32"},
            }});

            // depthwise_conv2d_buf: (sizeConst×2, buf×4, int2, int, int2×4, int×2) — 1.2.0 + 3.6.0
            add({"depthwise_conv", "depthwise_conv2d_buf_fp32", {
                TS{"1.2.0", "depthwise_conv2d_c4h1w4", 4,1, 3,0,2,5, 2, "identity_fp32"},
                TS{"3.6.0", "depthwise_conv2d_c4h1w4", 4,1, 3,0,2,5, 2, "identity_fp32"},
            }});
            ...
            add({"depthwise_conv", "depthwise_conv2d_subgroup_buf_fp32", {
                TS{"3.6.0", "depthwise_conv_2d_buf_c16_c16", 4,1, 10,0,2,3, 2, "identity_fp32"},
            }});
            ...
            add({"buffer_convert_buf", "buffer_convert_buf_fp32", {
                TS{"1.2.0", "nhwc_buffer_to_nc4hw4_buffer", 2,1, 3,0,2,0, 2, "identity_fp32"},
                TS{"3.6.0", "buffer_convert_to_buffer", 1,1, 0,0,2,1, 2, "identity_fp32"},
            }});
            ...
            add({"buffer_convert_subgroup_buf", "buffer_convert_subgroup_buf_fp32", {
                TS{"3.6.0", "nhwc_buffer_to_nc16hw16_buffer", 1,1, 8,0,2,0, 2, "identity_fp32"},
            }});

            // attention_buf: complex, simplified — 3.6.0
            add({"attention", "attention_buf_fp32", {
                TS{"3.6.0", "rearrange_qkv", 1,3, 12,0,2,0, 2, "identity_fp32"},
            }});

            // self_attention_buf: complex, simplified — 3.6.0
            add({"self_attention_buf", "self_attention_buf_fp32", {
                TS{"3.6.0", "split_transpose_qkv", 1,3, 8,0,2,0, 2, "identity_fp32"},
            }});
