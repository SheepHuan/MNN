    ...
    static struct Reg {
        Reg() {
            ...
            add({"cast", "cast_buf_fp32", {
                TS{"3.6.0", "cast_buf", 1,1, 1,0,2,0, 0, 2, "identity_fp32"},
            }});
            ...
            add({"unary", "unary_buf_fp32", {
                TS{"1.2.0", "unary_buf", 1,1, 1,0,3,0, 0, 3, "identity_fp32"},
                TS{"3.6.0", "unary_buf", 1,1, 1,0,2,0, 0, 2, "identity_fp32"},
            }});
            ...
            add({"conv", "conv_2d_buf_fp32", {
                TS{"1.2.0", "conv_2d_c4h1w1", 3,1, 2,0,2,5, 0, 2, "identity_fp32"},
                TS{"3.6.0", "conv_2d_1x1_local", 4,1, 6,0,2,0, 0, 2, "identity_fp32"},
            }});

            // conv_2d_c1_subgroup_buf: (sizeConst×2, buf×4, buf, int×13) — 3.6.0
            add({"conv", "conv_2d_c1_subgroup_buf_fp32", {
                TS{"3.6.0", "conv_2d_buf_subgroup_c1_c4_b2", 4,1, 13,0,2,0, 0, 2, "identity_fp32"},
            }});

            // conv_2d_c16_subgroup_buf: (sizeConst×2, buf×4, buf, int×13) — 3.6.0
            add({"conv", "conv_2d_c16_subgroup_buf_fp32", {
                TS{"3.6.0", "conv_2d_buf_subgroup_c16_c4_b2", 4,1, 13,0,2,0, 0, 2, "identity_fp32"},
            }});

            // conv_2d_int_buf: (sizeConst×2, buf×5, buf, int2, int×3, int2×4, int×3, float) — 3.6.0
            add({"conv", "conv_2d_int_buf_fp32", {
                TS{"3.6.0", "conv_2d_int_c4h1w1", 5,1, 3,1,2,5, 0, 2, "identity_fp32"},
            }});

            // depthwise_conv2d_buf: (sizeConst×2, buf×4, int2, int, int2×4, int×2) — 1.2.0 + 3.6.0
            add({"depthwise_conv", "depthwise_conv2d_buf_fp32", {
                TS{"1.2.0", "depthwise_conv2d_c4h1w4", 4,1, 3,0,2,5, 0, 2, "identity_fp32"},
                TS{"3.6.0", "depthwise_conv2d_c4h1w4", 4,1, 3,0,2,5, 0, 2, "identity_fp32"},
            }});
            ...
            add({"depthwise_conv", "depthwise_conv2d_subgroup_buf_fp32", {
                TS{"3.6.0", "depthwise_conv_2d_buf_c16_c16", 4,1, 10,0,2,3, 0, 2, "identity_fp32"},
            }});
            ...
            add({"buffer_convert_buf", "buffer_convert_buf_fp32", {
                TS{"1.2.0", "nhwc_buffer_to_nc4hw4_buffer", 2,1, 3,0,2,0, 0, 2, "identity_fp32"},
                TS{"3.6.0", "buffer_convert_to_buffer", 1,1, 0,0,2,0, 1, 2, "identity_fp32"},
            }});
