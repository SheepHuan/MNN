...
static void _cudaPrintExecutionClass(const Op* op, OpType dispatchType, const Execution* execution) {
    auto opName = op->name() ? op->name()->c_str() : "";
    auto className = _cudaExecutionClassName(execution);
    MNN_PRINT("[CUDAExecution] op=\"%s\", type=%s, dispatch=%s, execution=%s\n", opName, EnumNameOpType(op->type()),
              EnumNameOpType(dispatchType), className.c_str());
}
#endif
